...
def test():
    ...
    shape = (5,)
    cols = ['black', 'blue', 'green', 'orange', 'red']
    
    e = AccEvolution(shape, value=3., speed=.1, acc=.001, value_scale=1, speed_scale=.1, seed=0)
    #e.clip(7, 15)
    #e.set_cyclic(30)
    e.set_sine(average=.5, amplitude=.5)
    #e.clip(.2, .8)
    print(e)
    ...
    for i in range(shape[0]):
        v = [e(frame)[i] for frame in frames]
        ax.plot(frames, v, cols[i])
...
def test_locs():
    ...
    count = 500
    #v0 = dispersion(5, count, ndim=3, seed=None)
    steps = 3
    scale = .3
    seed = None
    v0 = random_curves(0, 1, shape=(5,), steps=steps, scale=scale, seed=seed)
    beziers = bezier.Beziers(v0)
    print(beziers)
    beziers.plot(points=False, curves=True)
    #v1 = random_curve(0, 1, steps=steps, scale=scale, in_disk=False, seed=seed)
    ...
    for i in range(5):
        ax.plot(v0[i, 0:, 0], v0[i, :, 1], '.-')
    #ax.plot(v1[0:, 0], v1[:, 1], '.-b')
    #ax.plot(v0[0:, 0], v0[:, 2], '.-')
