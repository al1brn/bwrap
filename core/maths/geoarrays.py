...
class GeoArray:
    def __init__(self, values=0., shape=None, copy=False):
        
        item_shape = self.item_shape()
        
        a = self.other_a(values, copy=copy)
    ...
    @staticmethod
    def other_a(other, copy=False):
        if issubclass(type(other), GeoArray):
            a = other.a
    ...
    # ---------------------------------------------------------------------------
    # Return new GeoArray of same type with the given shape
    
    def new(self, values=0., shape=None):
        if shape is None:
            shape = self.shape
        return type(self)(values, shape)
...
class Eulers(Vectors, Rotation):
    ...
    def __init__(self, values=0, shape=None, order='XYZ', copy=False):
        super().__init__(values, shape, copy=copy)
...
class Quaternions(WVectors, Coordinates, Rotation):
    ...
    def matrices(self):
        ...
        if False:
            ...
            m = Matrices(shape=self.shape)
            
            m.a[..., 0, 0] = qw2 + qx2 - qy2 - qz2
            m.a[..., 1, 0] = 2*(qxy - qwz)
            m.a[..., 2, 0] = 2*(qxz + qwy)
            
            m.a[..., 0, 1] = 2*(qxy + qwz)
            m.a[..., 1, 1] = qw2 - qx2 + qy2 - qz2
            m.a[..., 2, 1] = 2*(qyz - qwx)
            
            m.a[..., 0, 2] = 2*(qxz - qwy)
            m.a[..., 1, 2] = 2*(qyz + qwx)
            m.a[..., 2, 2] = qw2 - qx2 - qy2 + qz2
...
class AxisAngles(VectorsT, Rotation):
    ...
    def __init__(self, axis, shape=None, angles=None, copy=False):
        if angles is None:
            super().__init__(axis, shape=shape, copy=copy)
        else:
...
class TMatrices(GeoArray):
    ...
    def __init__(self, tmatrices=None, shape=None, locations=None, matrices=None, scales=None, copy=False):
        ...
        if tmatrices is None:
            ...
            empty = False
            super().__init__(tmatrices, shape=shape, copy=copy)
