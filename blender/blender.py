...
def create_object(name, what='CUBE', collection=None, parent=None, **kwargs):
    ...
    if what in generics:
        ...
        raise WError(f"Invalid object creation name: '{what}' is not valid.",
                ...
                parent = parent,
                kwargs = kwargs,
                valid_codes_are = generics + typeds)
...
def get_object(obj_or_name, mandatory=True, otype=None):
    ...
    if (obj is None) and mandatory:
        raise WError(f"Object '{obj_or_name}' doesn't exist",
                Function = get_object,
    ...
    if (obj is not None) and (otype is not None):
        if obj.type != otype:
            raise WError(f"Object type error: '{otype}' is expected",
                Function = get_object,
                obj_or_name = obj_or_name,
                mandatory = mandatory,
                otype = otype,
                blender_object_type = obj.type)
...
def get_create_object(obj_or_name, create=None, collection=None, **kwargs):
    ...
    return create_object(obj_or_name, what=create, collection=collection, parent=None, **kwargs)
