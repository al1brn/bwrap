...
class FTextChars():
    ...
    def array_of_chars(self):
        return [self.ftext.text_[self.ftext.chars_[i, 0]] for i in range(self.first_index, self.last_index+1)]
...
class FText():
    ...
    @property
    def array_of_chars(self):
        return [self.text_[i] for i in self.chars_[:, 0]]
    ...
    def format_char(self, char_indices, fmt, value):
        
        if fmt.upper() == 'BOLD':
            ifmt = FText.BOLD
        elif fmt.upper() == 'SHEAR':
            ifmt = FText.SHEAR
        elif fmt.upper() == 'XSCALE':
            ifmt = FText.XSCALE
        ...
        if char_indices is None:
            self.metrics[:, ifmt] = value
            self.dirty[:] = True
        else:
            self.metrics[char_indices, ifmt] = value
            self.dirty[char_indices] = True
    ...
    def get_char_format(self, index):
        return CharFormat(
